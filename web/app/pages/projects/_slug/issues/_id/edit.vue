<template>
    <ElForm label-position='left' :rules="rules" class="m-10 bg-white border-solid p-8" ref="form" :model="issue" label-width="150px">
        ...
        <ElFormItem label="Description" prop="description">
            <ElInput placeholder="Issue description" type="textarea" :rows="8" v-model="form.description"></ElInput>
        </ElFormItem>
        ...
        <hr>
        <ElFormItem>
            <ElButton class="float-right py-2 px-10" :loading="loading"  @click="update()" type="primary">Update</ElButton>
        </ElFormItem>
    </ElForm>
</template>
<script>
    ...
    export default {
        ...
        methods: {
            async getData() {
                ...
                this.form.priority = issue.priority;
                this.form.estimated_hours = issue.estimated_hours;
                this.form.actual_hours = issue.actual_hours;
                this.form.description = issue.description;
                this.form.status = issue.status;
                this.form.assign_id = issue.assign_id;
                this.form.category_id = issue.category_id;
                this.form.status = issue.status;
                this.form.milestone = issue.milestones[0].id;
                this.form.start_date = issue.start_date;
                this.form.due_date = issue.due_date;
                this.issue = issue;
            ...
            async update() {
                ...
                try {
                    ...
                    const { slug } = this.$route.params;
                    this.loading = true;
                    await update(slug, this.issue.id, this.form);
                    this.$router.push(`/projects/${slug}/issues/${this.issue.id}`);
